...
class FabricListItem extends HTMLElement {
        ...
        if (property == null || property === 'actions') {
            if (this._actions != null && Array.isArray(this._actions)) {
                if (this._refs.actions)
                    this._refs.actions.innerHTML = this._actions.map(action => '<div class="ms-ListItem-action" data-action="' + action.toString() + '"><i class="ms-Icon ms-Icon--' + action.toString() + '"></i></div>').join('');
            }
        }
    }
    __setListeners() {
    ...
    _actionClickHandler(e) {
        if (!e.target)
            return;
        const target = e.target.closest('.ms-ListItem-action');
        if (target && target.classList.contains('ms-ListItem-action')) {
