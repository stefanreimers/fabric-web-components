class FabricContextualMenu extends FabricContextualHost {
  ...
  __setupUI() {
    ...
    while (this.childNodes.length > 0) {
      ...
      if (fragmentUl) fragmentUl.appendChild(this.childNodes[0]);
    }
    ...
    // Inject nodes
    if (this._refs.main) this._refs.main.innerHTML = fragment.innerHTML;

    this._refs.menu = this.querySelector('.ms-ContextualMenu');
    [].forEach.call(this.querySelectorAll('.ms-ContextualMenu > li'), (entry: HTMLLIElement) => {
      ...
      if (entry.classList.contains('divider')) { entry.classList.add('ms-ContextualMenu-item--divider'); entry.classList.remove('divider'); }

    });
  }

  setMenuContent(content: string) {
    if (this._refs.menu) this._refs.menu.innerHTML = content;
  }
  ...
  __getStaticAttributes() {
    ...
    if (modifier && ['multiselect'].indexOf(modifier) !== -1) this._modifier = modifier;

    let host = this.getAttribute('host');
    if (host) this._host = host;

    let hostEvent = this.getAttribute('hostevent');
    if (hostEvent) this._hostEvent = hostEvent;
  }
  ...
  __setListeners() {
    ...
    if (hostNode == null) throw TypeError('No node available for host');
  ...
  setHost(host: string, event: string) {

    let h = host || this._host;
    if (!h) throw RangeError('No host given');
    //@ts-ignore
    if (typeof h === 'string') h = document.querySelector(h);

    let e = event || this._hostEvent;
    if (!e) throw RangeError('No event specified');
  ...
  __hostEventHandler(event: Event) {
