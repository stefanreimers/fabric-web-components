interface IFabricBreadcrumbItem {
  text: string;
  link?: string;
  tabIndex?: number
}
...
class FabricBreadcrumb extends HTMLElement {
  ...
  public readonly MEDIUM: number = 639;
  private _tabIndex: number = 2;
  private _currentMaxItems: number = 0;
  private _items: IFabricBreadcrumbItem[]
    ...
    if (this.children && this.children.length > 0) {
      ...
      while (this.children.length > 0) {
        ...
        if (this.children[0].tagName.toLowerCase() === 'a') {
          ...
          //@ts-ignore
          let x: IFabricBreadcrumbItem = { text: this.children[0].innerText }
          if ((<HTMLAnchorElement>this.children[0]).href.trim() !== '') x.link = (<HTMLAnchorElement>this.children[0]).href.trim();
          if (this.children[0].getAttribute('tabIndex')) x.tabIndex = parseInt(this.children[0].getAttribute('tabIndex') || '', 10);
          tempItems.push(x)
  ...
  removeOutlinesOnClick(e?: MouseEvent) {
    this._refs.list && this._refs.list.blur()
  }

  addItem(e: string, t: string) {
    this._items.push({ text: e, link: t }),
      this._updateBreadcrumbs()
  }
  removeItemByLabel(e: string) {
    for (var t = this._items.length; t--;)
      this._items[t].text === e && this._items.splice(t, 1);
    this._updateBreadcrumbs()
  }
  removeItemByPosition(e: number) {
    this._items.splice(e, 1),
      this._updateBreadcrumbs()
  }

  _resetList(e: HTMLElement) {
    for (; e.firstChild;)e.removeChild(e.firstChild)
  }
  _openOverflow(e: Event) {
  ...
  _overflowKeyPress(e: KeyboardEvent) {
    13 === e.keyCode && this._openOverflow(e)
  }
  _closeOverflow(e: Event) {
  ...
  _addBreadcrumbItems(e: number) {
    ...
    if (t = t < 0 ? 0 : t, t >= 0)
      for (t; t < this._items.length; t++) {
        ...
        //@ts-ignore
        if (n.link) n.setAttribute("href", n.link);
