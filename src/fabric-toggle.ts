class FabricToggle extends HTMLElement {
  ...
  set textleft(value) { if (this._textleft === !!value) return; this._textleft = !!value; this.__setProperties('textleft'); }

  get on() { return this._on }
  set on(val) { if (val != this._on) { this._on = val; this.__setProperties('on'); } }

  get off() { return this._off }
  set off(val) { if (val != this._off) { this._off = val; this.__setProperties('off'); } }

  get description() { return this._description }
  set description(val) { if (val != this._description) { this._description = val; this.__setProperties('description'); } }
  ...
  private __setProperties(property?: string) {
    ...
    //@ts-ignore
    if (property == null || property === 'disabled') { if (this._disabled === false) { this._refs.label.setAttribute("tabindex", "0") } else { this._refs.label.removeAttribute("tabindex") } }

    if (property == null || property === 'textleft') { this._refs.container.classList[(!!this._textleft) ? 'add' : 'remove']('ms-Toggle--textLeft'); }

    ['on', 'off', 'description'].forEach((prop) => {
      //@ts-ignore
      if (property == null || property === prop) { this._refs[prop].textContent = this[prop] || ''; }
    });
  ...
  attributeChangedCallback(attr: string, oldValue: string | null, newValue: string | null) {
    ...
    if (typeof this[attr] === 'boolean') {
      ...
      //@ts-ignore
      if (this[attr] != newValue) this[attr] = newValue;
    }
  }
