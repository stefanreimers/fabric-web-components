class FabricContextualMenu extends FabricContextualHost {
	...
	__setupUI(){
		...
		while (this.childNodes.length > 0) {
			fragment.querySelector('ul').appendChild(this.childNodes[0]);
		}		
		...
		// Inject nodes
		if(this._nodes.main) this._nodes.main.innerHTML = fragment.innerHTML;
		fragment = null;		
		
		this._nodes.menu= this.querySelector('.ms-ContextualMenu');
	...
	setMenuContent(content){
		if(this._nodes.menu) this._nodes.menu.innerHTML = content;
	}
	...
	__setListeners(){
		...
		if(!this._host) throw RangeError('No host defined');
		...
		hostNode.addEventListener('click', this.__hostClickHandler.bind(this) );
	}
	
	__hostClickHandler(event){
