class FabricRadioButton extends HTMLElement {
	...
	__addListeners(){
		this._nodes.field.addEventListener("focus", this._onFocusHandler.bind(this), false);		
		this._nodes.field.addEventListener("blur", this.__onBlurHandler.bind(this), false);
		this._nodes.field.addEventListener("click", this.__onClickHandler.bind(this), false);
		this._nodes.field.addEventListener("keydown", this.__onKeydownHandler.bind(this), false);
	}
	
	
	_onFocusHandler() {
      this._nodes.field.classList.add("in-focus");
    }

    __onBlurHandler() {
      this._nodes.field.classList.remove("in-focus");
    }
	...
	__setProperties(property){
		...
		if(property == null || property === 'disabled') { 
			this._nodes.input.disabled = !!this._disabled; 
			this._nodes.field.classList[(!!this._disabled)?'add':'remove']('is-disabled');
			this._nodes.field.setAttribute('aria-disabled',(!!this._disabled)?'true':'false' );
		}
		
		if(property == null || property === 'checked') { 
			this._nodes.input.checked = !!this._checked; 
			this._nodes.field.classList[(!!this._checked)?'add':'remove']('is-checked');
			this._nodes.field.setAttribute('aria-checked',(!!this._checked)?'true':'false' );
		}
		
		if(property == null || property === 'label') { this._nodes.label.textContent = this._label || ''; }
		if(property == null || property === 'name') { this._nodes.field.name = this._name || ''; }
