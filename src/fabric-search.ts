class FabricSearch extends HTMLElement {
  ...
  get value() { return (this._refs.input) ? (<HTMLInputElement>this._refs.input).value : null }
  //@ts-ignore
  set value(val) { if (this._refs.input && val != this.value) this._refs.input.value = val; }
  ...
  private __setProperties(property?: string) {
    ...
    //@ts-ignore
    if (property == null || property === 'disabled') { if (this._refs.input) this._refs.input.disabled = this._disabled; this._refs.container.classList[(this._disabled) ? 'add' : 'remove']('is-disabled'); }
  ...
  __addListeners() {
    ...
    if (this._refs.input) this._refs.input.addEventListener('input', (e) => {
      console.log('input', e);
      this.__setHasText();
    });

    if (this._refs.clear) this._refs.clear.addEventListener('click', (e) => {
      //@ts-ignore
  ...
  private __getStaticAttributes() {
    ...
    if (modifier && ['commandBar'].indexOf(modifier) !== -1) this._modifier = modifier;
