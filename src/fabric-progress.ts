class FabricProgress extends HTMLElement {
  ...
  private _refs: { [index: string]: HTMLDivElement | null };
  ...
  set name(val) { if (this._name === val) return; this._name = val; this.__setProperties('name'); }

  get description() { return this._description }
  set description(val) { if (this._description === val) return; this._description = val; this.__setProperties('description'); }

  get total() { return this._total }
  set total(value) { value = (typeof value === 'number') ? value : parseFloat(value); if (isNaN(value) || this._total === value) return; this._total = value; this.__setProperties('total'); }

  get progress() { return this._progress }
  set progress(value) { value = (typeof value === 'number') ? value : parseFloat(value); if (isNaN(value) || this._progress === value) return; this._progress = value; this.__setProperties('progress'); }
  ...
  private __setProperties(property?: string) {
    ...
    if (property == null || property === 'name') { if (this._refs.name) this._refs.name.textContent = this._name }
  ...
  __updateProgress() {
    ...
    if (this._refs.bar) this._refs.bar.style.width = percentage + "%";
  }
